...
CREATE TABLE Game(
   ...
   duration_level VARCHAR(20), -- ✅ must be here before trigger
   PRIMARY KEY(game_id)
);
...
CREATE TABLE Users(
   user_id INT NOT NULL AUTO_INCREMENT,   -- forgot the NOT NULL AUTO_INCREMENT BUT VERY IMPORTANT OTHERWISE CAN'T REGISTER NEW USERS
   password VARCHAR(50),
   email VARCHAR(50),
   username VARCHAR(50),
   PRIMARY KEY(user_id)
...
ALTER TABLE Game MODIFY game_image VARCHAR(255);
